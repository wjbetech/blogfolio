...
function Portrait({ className, sizes = "(max-width: 768px) 100vw, 400px" }: { className: string; sizes?: string }) {
  return <div className={`relative overflow-hidden bg-bg-300 ${className}`}><Image src="/images/assets/avatar.png" alt="William East in Seoul" fill priority unoptimized sizes={sizes} className="object-cover" /></div>;
}
...
function StudioHero() {
  return <section className="overflow-hidden border border-palette-border/65 bg-bg-200/35">
    <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)]">
      ...
      <div className="relative min-h-80 border-t border-palette-border/60 bg-bg-300 md:min-h-[34rem] md:border-l md:border-t-0"><div className="absolute inset-4 flex items-center justify-center sm:inset-6"><Portrait className="aspect-[506/478] w-full max-w-[506px]" sizes="506px" /></div><div className="absolute bottom-8 left-8 bg-bg-100/95 px-4 py-3 shadow-lg sm:bottom-10 sm:left-10"><span className="block font-mono text-[11px] uppercase tracking-[0.15em] text-accent-200">Current coordinates</span><span className="mt-1 block font-semibold text-headline">Seoul, South Korea</span></div></div>
    </div>
  </section>;
}
  ...
  return <section className="min-w-0 border-t border-accent-200/60 pt-7" aria-label={title}>
    ...
    <Carousel ref={carousel} hideControls gap={24} step={400}>
      {entries.map((entry) => <Link key={entry.href} href={entry.href} className="group block w-88 shrink-0 snap-start focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent-100">
        <article className="h-full overflow-hidden border border-palette-border/60 bg-bg-200/40 p-3">
          <div className="relative aspect-[7/4] overflow-hidden bg-bg-300"><Image src={entry.image || "/images/assets/placeholder.png"} alt="" fill sizes="(max-width: 640px) 304px, 352px" className="object-cover transition-transform duration-500 group-hover:scale-[1.04] motion-reduce:transition-none" /></div>
          <div className="px-2 pb-2 pt-4"><p className="mb-2 font-mono text-[11px] uppercase tracking-[0.16em] text-accent-200">{entry.kind}</p><h3 className="text-xl font-serif font-semibold leading-snug text-headline transition-colors group-hover:text-accent-100">{entry.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-relaxed text-paragraph">{entry.description}</p><span className="mt-4 inline-block text-sm font-semibold text-accent-200">View ↗</span></div>
        </article>
      </Link>)}
    </Carousel>
  </section>;
...
export default function StudioHomepage({ posts, projects }: { posts: BlogCardData[]; projects: ProjectCardData[] }) {
  ...
  return <div className="space-y-12 pb-10 pt-4 md:space-y-16">
    ...
    <Shelf title="Built at the desk" entries={projectEntries} href="/dev" linkText="All projects" />
  </div>;
}
